...
def processImage(file, fileName, backgrounds):
    ...
    if type(img) != type(None):
        ...
        bckgr = backgrounds[idx].copy()
        img_combi = cv2.bitwise_and(img,img, bckgr, mask= mask_threshold)
...
for file in inhoud:
    ...
    #print('Next ', str(file), ': ', os.path.isfile(_file))
    if os.path.isfile(_file):
        processImage(_file, file, backgrounds)
